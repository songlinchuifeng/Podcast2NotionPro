...
def queryNetSourceParse(task_id, dir_id, trans_list):
    ...
    if response.ok:
        ...
        if status == 0:
            urls = data.get("urls")
            for url in urls:
                if url.get("showName") in trans_list:
            ...
            time.sleep(1)
            return queryNetSourceParse(task_id=task_id, dir_id=dir_id,trans_list=trans_list)
        else:
...
def handle_title(title):
    """处理标题，通义转写的时候有的emoji表情会被去掉"""
    no_emoji_title = emoji.replace_emoji(title, replace="")
    return re.sub(r"\s+", " ", no_emoji_title).strip()
        ...
        if dir_id:
            ...
            for episode in value.get("episodes"):
                ...
                if title in records:
                    ...
                    trans_list.append(handle_title(title))
        else:
            trans_list = [
                utils.get_property_value(x.get("properties").get("标题"))
                for x in value.get("episodes")
            ]
            trans_list.extend([handle_title(x) for x in trans_list])
            need_trans = True
        if need_trans:
            if value.get("rss"):
